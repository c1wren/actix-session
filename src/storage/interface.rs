...
pub enum LoadError {
    ...
    /// Something went wrong when retrieving the session state from redis.
    #[display(fmt = "Something went wrong when retrieving the session state from redis.")]
    Redis(RedisError),
}
...
impl std::error::Error for LoadError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::Deserialization(e) => Some(e.as_ref()),
...
pub enum SaveError {
    ...
    Generic(anyhow::Error),
    /// Something went wrong when persisting the session state in redis.
    #[display(fmt = "Something went wrong when persisting the session state in redis.")]
    Redis(RedisError),
}
...
pub enum UpdateError {
    ...
    Generic(anyhow::Error),
    /// Something went wrong when updating the session state in redis.
    #[display(fmt = "Something went wrong when updating the session state in redis.")]
    Redis(RedisError),
}
